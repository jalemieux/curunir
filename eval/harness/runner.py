"""Generic graded-eval runner — persona-agnostic engine.

A persona suite supplies a `SuiteConfig` (its task list, a name/title, and where
to write results / find fixtures) and calls `main(cfg)`. The runner sends each
task to a running Curunir instance over the WebSocket channel, captures a
`Result`, grades it with `eval.harness.graders`, and writes a status table plus
markdown/JSON/HTML reports. Unlike `eval/run_evals.py` (capture-only, human
eyeballed), this scores each task pass/fail/pass-slow with a one-line reason.

This was extracted verbatim from the finance runner; `run_finance_evals.py` is
now a thin shim that builds a finance `SuiteConfig` and calls `main()`. The
graders live in `eval.harness.graders` (re-exported here as `G`).

Prereqs (the SUT runs the persona being evaluated):
...
from eval.harness import graders as G

REPO_ROOT = Path(__file__).resolve().parents[2]
# The SUT has a single context/memory/, regardless of persona; fixture seeding
# stashes it here and restores on exit.
...
@dataclass
class SuiteConfig:
    """Everything that differs between persona eval suites.

    name      — short slug; the report filename prefix (`finance-…`, `default-…`)
    title     — human title shown in the HTML header and <title>
    tasks     — the task list (see eval/<persona>/<persona>_tasks.py)
    results_dir — where JSON/MD/HTML reports are written
    fixture_memory_dir — `fixtures/memory/` for this suite; only used by --fixture
                  (may be None for suites with no memory fixtures)
    """
    ...
    tasks: list
    results_dir: Path
    fixture_memory_dir: Path | None = None


def seed_fixture(name: str, fixture_memory_dir: Path):
    """Stash the real `context/memory/` and seed `<fixture_memory_dir>/<name>/`.

    Returns whether a real-memory stash was created (so restore knows to put it
    back). Fixture-dependent tasks cross the memory boundary — the agent reads
    its STORED state and computes — which is the only way to measure the
    memory-schema behavior. The SUT reads memory on demand via tools, so
    swapping the directory on disk before sending prompts is enough; no restart.
    """
    import shutil

    src = fixture_memory_dir / name
    if not src.is_dir():
...
async def main_async(cfg: SuiteConfig, args) -> None:
    tasks = select(cfg, args)
    if not tasks:
        print("No tasks matched the filter.")
        return

    # Fixture-seeded memory (some tasks read the STORED state). Only ever for a
    # LOCAL SUT — we cannot touch a remote filesystem — and always restored,
    # even on error, by the try/finally below.
    stashed = None
    if args.fixture:
        if cfg.fixture_memory_dir is None:
            raise SystemExit(f"--fixture is not supported by the {cfg.name!r} suite "
                             "(no fixture_memory_dir configured).")
...
async def _run_suite(cfg: SuiteConfig, args, tasks: list[dict]) -> None:
    ...
    _report(cfg, model, rows, getattr(args, "interface", None))


def _report(cfg: SuiteConfig, model: str, rows: list[dict], interface: str | None = None) -> None:
    counts: dict[str, int] = {}
    for r in rows:
    ...
    # Tag the interface into the slug so multi-mode runs land in separate files
    # and diff cleanly — the suite is identical across modes; only the SUT's
    # surface differs (see the finance A/B/D coordination note).
    ...
    (cfg.results_dir / f"{cfg.name}-{ts}-{safe}.json").write_text(
    ...
    (cfg.results_dir / f"{cfg.name}-{ts}-{safe}.md").write_text("\n".join(md) + "\n")

    # HTML: the primary human report — collapsible per-task trace, filters.
    html_path = cfg.results_dir / f"{cfg.name}-{ts}-{safe}.html"
    html_path.write_text(_html_report(cfg, payload, counts, summary))
...
def _html_report(cfg: SuiteConfig, payload: dict, counts: dict, summary: str) -> str:
    ...
    return ("<!doctype html><html lang='en'><head><meta charset='utf-8'>"
            ...
            f"<title>{_esc(cfg.title)} — {_esc(payload['model'])}</title>{_CSS}</head><body>"
            f"{header}<main id='list'>{cards}</main>{_JS}</body></html>")


def build_parser(cfg: SuiteConfig) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=f"Run graded {cfg.name}-persona evals")
    ...
    p.add_argument("--interface", choices=["cli", "tool"],
                   help="label this run's surface (the SUT's adapter is configured "
                        "separately); tags the report so a multi-mode comparison diffs cleanly")
    p.add_argument("--verbose", "-v", action="store_true",
                   help="stream each task's tool calls and text live as it runs")
    return p


def main(cfg: SuiteConfig, argv: list[str] | None = None) -> None:
    """Entry point a persona shim calls with its SuiteConfig."""
    args = build_parser(cfg).parse_args(argv)
    if args.list:
        list_tasks(select(cfg, args))
        return
