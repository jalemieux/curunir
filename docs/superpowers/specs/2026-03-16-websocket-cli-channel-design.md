...
**`start()`:**
- Binds to `{host}:{port}` (default `0.0.0.0:8765`)
...
- On disconnect: logs it, keeps listening for reconnect. Agent continues running — no session loss.
...
The server owns `channel`, `session_id`, and `reply_address` — the client doesn't set or see these. The client sends user content and optional commands; the server wraps them into `IncomingMessage`.
...
- `/verbose`: handled locally (toggles tool call display), not sent to server
- EOF (Ctrl-D): sends disconnect, exits cleanly
...
**Reconnection:**
- On disconnect: prints status, attempts reconnect with exponential backoff
...
- Add `ws_host: str` (env `WS_HOST`, default `"0.0.0.0"`) and `ws_port: int` (env `WS_PORT`, default `8765`) to `AgentConfig`
