...
`Agent.handle()` is the heart: receive message → trim history (250k char limit) → build system prompt → call LLM (via LiteLLM) → execute the tool-call batch concurrently → loop (max 200 iterations) → return response. Tool calls in a single batch run in parallel via `asyncio.gather()`; results map back 1:1 to `tool_call_id` to satisfy the chat schema. History char estimation charges a fixed ~2000 chars per image block so image-heavy sessions age out of the window alongside text, and counts assistant tool-call `arguments` (where `write`/`edit` embed entire file bodies, uncapped) so write/edit-heavy sessions trim correctly instead of measuring as ~0.

**Tool-executor backstop.** `_run_tool_call` wraps `execute_tool_call` in `try/except Exception`: any tool that raises an unanticipated exception is logged at `warning` and returned as a model-visible `role: tool` error (`"Error: tool '<name>' failed: ..."`, capped via `_cap_tool_result`) instead of propagating. Because `asyncio.gather` runs with `return_exceptions=False`, an uncaught raise here would otherwise escape `handle()` and kill the whole turn/session — so no single tool can crash the loop, and the model can route around the failure. The same `try/except Exception` also wraps the `on_tool_call` streaming callback (best-effort UI notification): a callback raise is logged and swallowed so a broken UI socket can't escape `gather` and kill the turn either.

**Schema-valid-transcript backstop (defense-in-depth).** The assistant `tool_calls` message is appended to `history` *before* the batch runs, so if any exception (or `BaseException`, e.g. `CancelledError`) escapes the whole batch dispatch, the loop would unwind leaving a dangling `tool_calls` message with zero `role: tool` responses — which `agent_worker` then persists, permanently bricking the session (every later turn fails provider "tool_use without tool_result" validation). The `gather` + append loop is therefore wrapped so that on any escape, `_stub_missing_tool_responses` appends a stub `role: tool` reply for every unanswered `tool_call_id` (idempotent — keyed on which ids already have a response) **before** re-raising. So a persisted transcript is always chat-schema-valid, even when the turn itself errors out.
...
**Transient/mid-stream retry (`src/llm.py::call_llm`).** The retry loop wraps both `litellm.acompletion` *and* `_consume_stream`, so a **mid-stream** drop (the network read happens while draining the iterator, not when `acompletion` returns) is catchable and retryable — this is what let an OpenRouter upstream-idle-timeout `MidStreamFallbackError` escape and kill the turn before. `_is_retryable` classifies retryable failures by status (`429/502/503`) plus exception class (`Timeout`/`APIConnectionError`/`ServiceUnavailableError` — the last covers `MidStreamFallbackError`), resolved via `getattr(litellm, ...)` for version-safety. To avoid duplicating output, a stream is retried **only before any visible text delta has been emitted** (the reported idle timeout fires pre-content, so it's covered); once text has streamed, the error re-raises. A client-side `LLM_REQUEST_TIMEOUT` (default 120s) is forwarded to litellm/httpx as the inter-chunk idle bound so curunir fails fast on a hung upstream rather than waiting on the provider's longer idle limit.
...
- **Portal** (`portal.py`): Outbound WebSocket to a hosted portal (`CURUNIR_PORTAL_URL` + `CURUNIR_PORTAL_TOKEN`). Container dials portal; portal multiplexes browser ↔ container. Session ID is `"portal"`. See `portal/` directory for the portal service. **Heartbeat.** The socket idles between agent turns, so a proxy idle timeout (~25–36s) would hang it up and the container would flap (dropped deltas + reconnect latency). The portal server drives an **application-level heartbeat** — server→container `{"type":"ping"}` every `PORTAL_WS_HEARTBEAT_SEC` (default 15s, env-overridable in the portal); the container answers `{"type":"pong"}`. Both ends treat `ping`/`pong` as silent no-ops (no agent turn, no "unknown type" warning). Data frames are used rather than protocol pings so the keepalive works even if the proxy ignores WS control frames; `PortalChannel` also sets explicit protocol-level `ping_interval`/`ping_timeout` as belt-and-suspenders.
- **Local Web UI** (`local_web.py`): An operator-only web console served *from* the container itself — the co-located counterpart to the remote Portal. Off by default; enabled by `LOCAL_UI_ENABLED`. Hosts a **FastAPI + uvicorn** app bound to `LOCAL_UI_HOST` (default `127.0.0.1`, port `LOCAL_UI_PORT` default `8766`). Session ID is fixed `"local"`. Two halves:
  ...
  **Read panels (the unique co-located value).** Read-only REST endpoints (`/api/usage`, `/api/portfolio`, `/api/crm`, `/api/schedules`, `/api/memory`, `/api/memory/file`) backed by thin adapters in `src/local_ui/readers.py` over the *existing* read APIs (`UsageStore.summary`, `portfolio.engine.*`, `crm.engine.*`, `scheduler._load_tasks`, sandboxed `context/memory/` walks). No new query logic — the UI can't drift from `python -m src.usage` / `portfolio.py` / `crm.py`. `memory_file` is path-traversal guarded (rejects `..`/absolute paths that escape `context/memory/`).
  ...
  The **Balance Sheet** tab is a single-page dashboard (not the old flat tables): a net-worth hero with a *values-as-of* staleness caveat (the `value_asof` range, since stored values aren't necessarily live), an **allocation bar** rendering `rollup()`'s positive buckets as net-worth composition (debt shown as a separate Liabilities line), **holdings grouped by class** into collapsible sections with per-class columns and subtotals (retirement folds under Equities, mirroring the rollup), and a **trades + realized-P&L** section (`trade_history` + `realized_pnl`) scoped to the current fiscal/calendar year. `portfolio_overview()` is extended to also return `unrealized`/`trades`/`realized`/`as_of`; the math stays in `engine` (`engine.unrealized` added) so the UI never re-sums. Read-only — all writes stay in chat/CLI. **Persona-gated:** the Balance Sheet tab and its `/api/portfolio` endpoint are persona-specific *modules* — rendered only on personas that own them, derived from the skill allowlist by `src/modules.enabled_modules()` (a module is enabled iff its gating skill — `balance-sheet` for portfolio — is explicitly named in the persona's allowlist; a `None`/empty allowlist like `default`'s enables **no** modules). `src/modules.py` is the single source of truth tying *module → gating skill → panel id → endpoint prefixes*; `LocalWebChannel` advertises enabled panel ids in the `meta` frame (the SPA strips `[data-module]` tabs/panels not in `meta.modules`) and 404s gated endpoints as a server-side backstop (after the token check, so an unauthenticated probe still gets 401 and can't enumerate modules).

  The **CRM** tab is the marketing analog of the Balance Sheet tab, backed by `crm_overview()` → `/api/crm` (`available=False` with empty payloads until a lead exists). It renders pipeline-by-stage cards, leads grouped by stage into collapsible sections, and a recent-activity (interaction ledger) panel — modeled on `loadPortfolio`, reusing the same group/badge styling. Read-only — all writes stay in chat/CLI/tool. **Persona-gated** exactly like the Balance Sheet tab: CRM is a second `src/modules.py` entry (`crm` module, gating skill `crm`, panel `crm`, endpoint `/api/crm`), so the tab + endpoint render only on personas whose allowlist names the `crm` skill — marketing (which alone allowlists `crm`) shows it; finance and `default` do not. The `/api/crm` endpoint 404s when the module is disabled (after the token check, so an unauthenticated probe still gets 401).
  ...
  **Outbound delivery guarantee (buffer-and-replay + idempotency).** A chat/slash message composed while the socket is mid-reconnect used to be silently dropped: `connection.js` `send()` returned `false` for a not-`OPEN` socket and `chat.js` ignored it (rendering a phantom bubble + spinner for a frame the server never received). Now durable frames (`durable: true`, stamped with a `client_msg_id`) are buffered in `connection.js`'s `outbox` when the socket is closed and flushed in order on reconnect (`routeOutbound`/`drainOutbox`, pure helpers tested in `tests/js/test_connection_outbox.mjs`); `chat.js` only paints the optimistic bubble once `send()` confirms delivery-or-buffer. Because a buffered frame can be replayed, `LocalWebChannel._seen_msg` dedups on `client_msg_id` via a bounded recent-id ledger (`_RECENT_MSG_CAP`) before `in_queue.put` — a missing id is never deduped (back-compat). Control/read frames (interrupt, history/skills/conversations requests) stay unbuffered since they're re-issued on reconnect anyway.

  **Files rail (deliverables, download-only).** The Chat tab gains a right-rail `<aside>` (scoped to that tab; read panels stay full-width, collapsed by default with the open/closed preference in `localStorage`) that lists `context/workspace/generated/` — where the agent writes user-facing deliverables (memos, theses, exported PDFs) that otherwise sit trapped on disk behind a flaky LLM-in-the-loop `attach` step. Two token-gated read routes back it: `GET /api/files` (flat, newest-first listing via `readers.generated_files`; the rail groups by day client-side) and `GET /api/files/download?path=<rel>` (streams one file via `FileResponse` with `Content-Disposition: attachment`, resolved through the path-traversal-guarded `readers.generated_file_path` — same `.resolve()`-collapses-symlinks guard as `memory_file`). The token check stays **before** the 400/404 mapping so an unauthenticated probe can't enumerate files. Read-only, not persona-gated (deliverables aren't a vertical module) — a universal Chat-tab affordance with no `data-module` tag; all writes stay in chat/CLI/tools. Row click navigates straight to the download URL (no agent, no `attach`, no LLM in the path); the rail refreshes on open, on manual ⟳, and (if open) on turn-final so a just-generated file appears.
...
**Shared helpers.** `fastmail.py` (IMAP/SMTP client: envelope→dict normalization, thread-root derivation, MIME assembly, recipient allowlist, stable reply Message-IDs), `_render.py` (the one markdown→styled-HTML renderer — `render_html` + the HTML/CSS wrapper — shared by the inbound reply path (`EmailChannel.send`) and the proactive send CLI (`skills/email-send/email_send.py`), so every email auto-renders pretty HTML at the transport boundary; returns `""` when `markdown` is missing so callers fall back to text-only via `... or None`), `_attachments.py` (decode/stage/enrich pipeline shared by WS + Portal, with symlink / Windows-reserved-name / Unicode-normalization defenses and per-type size caps), and `_email_state.py` (atomic watermark persistence) back the channels above.
...
**Path placeholders.** Skill and persona markdown never spells `context/…` literally. Private, per-agent paths are `{{context}}/…` (`memory/`, `identity.md`, `conversations/`, `schedules.db`, `skills/`) and container-shared paths are `{{shared}}/…` (`workspace/`, `uploads/`). `src/skills.py::render_paths` substitutes `AgentConfig.path_vars` wherever markdown enters a prompt — `load_skill(..., paths=config.path_vars)` and `build_static_prompt` (which renders identity, persona prompts and the manifest in one pass). With the legacy single-agent layout both render to `context`, so prompts are byte-identical to before. `tests/test_path_placeholders.py` lints `skills/**/*.md` and `personas/*/prompts/*.md` for raw literals; frontmatter `description`/`portal_summary` stay prose because `/help` and the portal show them unrendered. Reference files the model opens with `read` are not rendered, so they describe paths in words. Design: `docs/superpowers/specs/2026-09-26-agents-and-containers-design.md`.
...
**Positioning-test variants (`/v/<slug>/`).** `portal/static/v/<slug>/index.html` is served at `/v/<slug>/` with no per-variant code; each variant's beta form posts `source="<slug>"` so admin counts sign-ups per variant. The marketing persona adds variants through PRs labeled `gtm:landing` (flow in `skills/gtm-smoke-test/references/landing-page.md`), and the `gtm-landing-scope` CI check fails any such PR that touches files outside `portal/static/v/`. The builder merges (which deploys) and runs the ads — a documented handoff, not persona-held deploy/ad credentials (#544).
...
Local directory containing `identity.md` (agent persona, required), `memory/` (persistent facts), and the runtime SQLite stores `schedules.db` (cron tasks; see Scheduling), `portfolio.db`, `crm.db` (leads + pipeline; see CRM Engine), and `usage.db`.

**Two roles, one directory today.** `AgentConfig.context_dir` is the agent's *private* area (identity, memory, conversations, schedules, user skills) and `AgentConfig.shared_dir` is the container's *shared* area (`workspace/` deliverables, `uploads/`, `usage.db`, `.ws-token`, `email_state.json`). A bare `AgentConfig()` is the legacy layout where both are `./context` and every path field keeps its historical default; `AgentConfig.for_agent(name, context_dir, shared_dir)` derives `identity_file`/`schedules_db`/`portfolio_db`/`crm_db`/`skill_dirs[1]` from `context_dir` and `usage_db` from `shared_dir` (a multi-agent container gives each agent `context/agents/<name>/` with `context/` shared). `run.py` takes the pairing token, uploads dir and email state from `shared_dir`; `readers._generated_root` (the Files rail) reads the shared workspace. The bash tool exports `CURUNIR_CONTEXT_DIR`/`CURUNIR_SHARED_DIR` (absolute) so skill scripts (`portfolio.py`, `crm.py`, `snapshot.py`, `ingest.py`) default their stores to the running agent; the fs tools resolve relative paths against `config.repo_root`, like bash's cwd. The system prompt reads `identity.md` (then layers `personas/<active>/prompts/*.md` on top — see Personas). **`behavior.md` is no longer read at boot** — framework behavior moved into the persona bundle's `prompts/`; a stray `context/behavior.md` has no effect. The `/identity` skill only edits `identity.md`. Use `sync-context.sh` to rsync from a remote machine before starting.
...
- `WEBCAM_DEVICE` (default `/dev/video0`), `WEBCAM_RESOLUTION` — camera node or `http://`/`rtsp://` stream URL read by the `webcam` skill's `skills/webcam/snapshot.py`, which captures one frame with ffmpeg into `context/workspace/generated/` and describes it via `VISION_MODEL` (falling back to `MODEL` only if it supports vision). The device is mapped in by the `docker-compose.webcam.yml` override, applied only on the camera-bearing instance so camera-less deployments keep starting
...
- `CURUNIR_CONTEXT_DIR` / `CURUNIR_SHARED_DIR` — **not operator-set.** Exported by the `bash` tool into every skill-script subprocess (absolute paths of `AgentConfig.context_dir` / `shared_dir`) so `skills/*/…py` default their store paths to the running agent. Unset at a human shell → the scripts fall back to `context/`
