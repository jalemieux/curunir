...
- **Local Web UI** (`local_web.py`): An operator-only web console served *from* the container itself — the co-located counterpart to the remote Portal. Off by default; enabled by `LOCAL_UI_ENABLED`. Hosts a **FastAPI + uvicorn** app bound to `LOCAL_UI_HOST` (default `127.0.0.1`, port `LOCAL_UI_PORT` default `8766`). Session ID is fixed `"local"`. Two halves:
  ...
  **Read panels (the unique co-located value).** Read-only REST endpoints (`/api/usage`, `/api/portfolio`, `/api/schedules`, `/api/memory`, `/api/memory/file`) backed by thin adapters in `src/local_ui/readers.py` over the *existing* read APIs (`UsageStore.summary`, `portfolio.engine.*`, `scheduler._load_tasks`, sandboxed `context/memory/` walks). No new query logic — the UI can't drift from `python -m src.usage` / `portfolio.py`. `memory_file` is path-traversal guarded (rejects `..`/absolute paths that escape `context/memory/`).

  **Schedule editing + run-now (the write surface).** Five token-gated mutating routes — `POST /api/schedules` (create), `PUT /api/schedules/{id}` (edit cron/prompt/skill/enabled), `POST /api/schedules/{id}/toggle`, `DELETE /api/schedules/{id}`, and `POST /api/schedules/{id}/run` (fire on demand) — back the Schedules tab. The first four delegate straight to `schedule_store.engine` (`create`/`update`/`toggle`/`delete`), passing `skill_allowlist=config.skill_allowlist` exactly as the `schedule` tool does; they only map `ValueError` → HTTP 400, so all validation (cron, duplicate id, skill allowlist) stays in the engine and can't drift from the `schedule` tool or the scheduler. The SPA's Schedules tab is correspondingly editable (per-row Run-now/Edit/Enable-Disable/Delete + a New-schedule form, with a client-side next-runs cron preview that is convenience-only — the server stays authoritative). `id` is immutable (no engine rename); changing one is delete + create. The **Run now** route fires the task through the shared `scheduler.fire_task` node with `record_run=False` — same firing path as the scheduler (skill prepend, system-task mode, `sched:<id>:<ts>` session) but a *test-fire* that doesn't stamp run-metadata, so the cron cadence is untouched; it's fire-and-forget (returns `202 {"status":"started"}`), runs even if the task is disabled, and `404`s on an unknown id. Editing/running is loopback-bound and token-gated — the same trust boundary as the chat bridge, which can already drive the agent — so it widens no exposure.
...
The scheduler evaluates the table every ~60s via croniter (re-querying each tick, so edits take effect without restart). When due, it stamps `last_attempt_at` via `mark_attempt` *before* dispatch (so a slow/crashed task doesn't re-fire), then dispatches `scheduler.fire_task` — the single reusable firing node that prepends the named skill's `SKILL.md`, runs `handle()` in system-task mode under a per-run `sched:<id>:<ts>` session id, and (with `record_run=True`) stamps `last_run`/`last_status` via `mark_run` only on success. The local-UI **Run now** button calls the *same* `fire_task` with `record_run=False`, so a manual test-fire takes the identical path without touching run-metadata or the cron cadence (see Local Web UI). `fire_task` swallows `handle()` exceptions (logging them) so a fire-and-forget background run can't crash its caller.
...
- `EMAIL_ENABLED`, `DEADSIMPLE_API_KEY`, `DEADSIMPLE_INBOX_ID`, `DEADSIMPLE_API_BASE` (default `https://api.deadsimple.email`), `EMAIL_ALLOWED_SENDERS`, `EMAIL_RESTRICT_OUTBOUND`, `EMAIL_POLL_INTERVAL`, `EMAIL_STATE_FILE`, `EMAIL_SEND_MAX_RETRIES` (default 5), `EMAIL_SEND_RETRY_BACKOFF` (default 30s), `EMAIL_FAILURE_ALERT_THRESHOLD` (default 5) — for the deadsimple.email channel (no Google/Gmail vars anymore)
