...
- `VISION_MODEL` — fallback vision model when `MODEL` is text-only. At boot, `litellm.supports_vision(MODEL)` is checked; if false, image attachments are described by `VISION_MODEL` and the description is sent to `MODEL` as text. If unset, images become a `[file (image, NKB) — no vision model configured]` text marker. `VISION_API_BASE` — endpoint for `VISION_MODEL` when it is served separately from `MODEL` (e.g. a local llama.cpp VL model on the Docker host, reached via `host.docker.internal`); `API_BASE` belongs to `MODEL` and is never reused for the vision call. Both the attachment pre-pass and the `webcam` skill's `snapshot.py` honor it.
