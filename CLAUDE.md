...
**Idle-session eviction (bounded memory).** In-memory session state spans six per-session dicts (`sessions`, `_session_prompts`, `_session_started_at_cache`, `_cancel_events`, `_session_tools`, `_last_access`); `_evict_session` is the single cleanup primitive that pops all of them, used by the system-task return sites and by `evict_idle_sessions()`. `handle()` stamps `_last_access` (monotonic) at turn start; a `run.py` background sweep (`periodic_session_eviction`, ~60s) evicts sessions idle longer than `session_idle_ttl_sec` (env `SESSION_IDLE_TTL_SEC`, default 1800s; `<= 0` disables), skipping `_running_sessions`. Eviction is invisible to users: transcripts persist in `context/conversations/` and `handle()` lazily rehydrates on next access — the only session-scoped loss is skill-unlocked opt-in tools, same as a process restart (the model re-runs `load_skill`). Eviction is synchronous (pure dict pops), so it's atomic w.r.t. an in-flight turn on the event loop.
...
- `SESSION_IDLE_TTL_SEC` — seconds a session may idle before its in-memory state is evicted (default 1800; `0` disables). Transcripts persist on disk and rehydrate transparently on the next message; this only bounds RSS growth on long-running deployments
