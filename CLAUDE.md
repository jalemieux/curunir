...
**Brokerage adapters (pluggable — pull live positions into the engine).** `src/portfolio/brokers/` is a config-only integration layer: a deployment enables zero, one, or several brokerages via the `BROKERAGE_ADAPTERS` env var alone (comma-separated; empty → none). `base.py` defines the `BrokerageAdapter` protocol (`name`, three-step interactive auth `auth_start`/`auth_complete`/`auth_status`, `fetch_accounts`/`fetch_positions`) and the normalized `BrokerAccount`/`BrokerPosition` dataclasses plus `BrokerError`/`BrokerAuthRequired`. `registry.py::enabled_adapters(env)` builds the enabled set (unknown name → `ValueError`; a known adapter missing its keys is *warned and skipped*, mirroring the persona key-warning). The first adapter is **E*TRADE** (`etrade.py`): OAuth 1.0a signed by the stdlib `_oauth1.py` signer (HMAC-SHA1, zero new deps, RFC-vector tested), tokens persisted to `context/.etrade-tokens.json` (mode 0600); E*TRADE tokens die at midnight ET, so a hard-expired token surfaces as `BrokerAuthRequired` pointing at the web console. **Reconcile, never overwrite** (`reconcile.py`): `broker_diff(path, positions)` buckets broker positions vs. local lots keyed on `(account, ticker)` — broker qty vs. the **sum of local lots** (broker positions are aggregate; the local model is specific-lot) — into matched / price-stale / qty-drift / missing-local / missing-remote, plus a derived **possible_account_mismatch** (a ticker in *both* missing-local and missing-remote, i.e. the same holding under a manual account label vs. the broker's opaque account id); `broker_apply(path, positions, source)` is conservative: it re-prices matched/stale lots and inserts brand-new tickers as one `extra.source=<adapter>`-tagged lot (surfacing `add_asset`'s dedup warnings), and only **reports** qty drift, missing-remote, and account-mismatch — the last is explicitly **not inserted** (that would double-count net worth; the operator relabels the local lot's `account` to the broker id, then re-syncs) — apply is idempotent. `service.py` is the reusable orchestration node (`status`/`accounts`/`diff`/`sync`/`find`) shared by every surface, folding per-adapter auth/network failures into `needs_reauth`/`error` fields rather than raising. Three surfaces reach it: the opt-in `portfolio` tool (`broker_accounts`/`broker_diff` reads, `broker_sync` write), the **Local Web UI's Brokerage section** (the *only* place the interactive OAuth verifier flow can be completed — the operator has no CLI), and a scheduled daily sync (the `schedule` tool + `balance-sheet` skill, no scheduler changes). Out of scope: aggregators/Plaid, lot-level cost-basis fidelity (v1 stores one aggregate lot for new tickers), force-reconcile of qty drift, wash-sale/FIFO.
