...
### Financial-Plan / Projection Engine (`src/financial_plan/`)

Deterministic retirement/goal **projection** engine, powering the finance persona's *"will my money last?"* answer with the same "math is tool-backed, never prose" rule as the balance sheet — but **stateless: there is no store**. Projection is pure computation, so `engine.py` takes plan inputs as a dict and returns structured results; no SQLite, no `db.py`. Pure functions: `project()` (year-by-year cash flow, accumulation→distribution, with depletion detection), `monte_carlo()` (seeded — `random.Random(seed)`, **no numpy**; draws annual *real* returns from Normal(mean, vol) per path and reports probability-of-success against the `SUCCESS_THRESHOLD = 0.85` bar plus ending-balance/depletion-age percentiles), `stress_grid()` (re-runs project + MC for four canned shocks: retire 2yr early, −20% year-1 return, +20% spending, late-life LTC one-time withdrawal), and `render_markdown()`. Conservative defaults are module constants with rationale comments — returns are **real (inflation-adjusted), not nominal** (`DEFAULT_REAL_RETURN = 0.05`, `DEFAULT_VOLATILITY = 0.12`); never raise them to make a plan "pass". Determinism (fixed seed + inputs → reproducible `success_prob`) is what lets the eval harness anchor against it. Starting balances flow in from the *existing* portfolio engine (`portfolio.engine.networth`) via the CLI's `--from-portfolio`, so the two engines stay decoupled but composable. Three surfaces reach it, mirroring the portfolio trio: the opt-in `financial_plan` tool (`src/tools/financial_plan_tool.py`, `{action, args}` → JSON, actions `project`/`montecarlo`/`stress`/`report`; unlocked by the `financial-plan` skill), a CLI (`skills/financial-plan/plan.py`), and the `financial-plan` skill itself. Out of scope: taxes, Social Security, RMDs, account-type (pre-tax vs Roth) differences — pre-tax real-dollar projection only.
