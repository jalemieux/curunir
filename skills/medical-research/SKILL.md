...
1. Write the full report as markdown to `{{shared}}/workspace/generated/{topic-slug}-{YYYY-MM-DD}.md` (e.g. `metformin-safety-2026-05-18.md`, `cbc-results-2026-05-18.md`). The H1 inside the document should be a descriptive title, not the filename slug. Include the disclaimer from the top of this skill. End the document with a `## Sources` section (see Inline citations below).
2. Convert it to PDF with pandoc — already installed in the environment:

   ```bash
   pandoc {{shared}}/workspace/generated/{topic-slug}-{YYYY-MM-DD}.md -o {{shared}}/workspace/generated/{topic-slug}-{YYYY-MM-DD}.pdf
   ```

   Never `pip install` a PDF library — pandoc is the tool. Do not render via HTML, headless Chromium, or CSS.
3. Attach the **PDF**: `attach(path="{{shared}}/workspace/generated/{topic-slug}-{YYYY-MM-DD}.pdf")`. If pandoc fails, attach the `.md` as a fallback.
