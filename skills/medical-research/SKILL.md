...
2. Convert it to PDF with the shared helper — pandoc is already installed in the
   environment, and the helper sanitizes LaTeX-hostile emoji then renders via
   xelatex→pdflatex:

   ```bash
   python -m src.md2pdf context/workspace/generated/{topic-slug}-{YYYY-MM-DD}.md
   ```

   Never `pip install` a PDF library — `src.md2pdf` (pandoc under the hood) is the tool. Do not render via HTML, headless Chromium, or CSS.
3. Attach the **PDF**: `attach(path="context/workspace/generated/{topic-slug}-{YYYY-MM-DD}.pdf")`. If PDF conversion fails, attach the `.md` as a fallback.
