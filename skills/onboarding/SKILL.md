---
name: onboarding
description: "Use on a not-yet-onboarded user's first message (the agent loop auto-triggers this when the identity file is absent) or when the user types `/onboarding` to redo setup. Runs profile → preferences → personality; the personality step writes the identity file, which is what marks onboarding complete."
---
