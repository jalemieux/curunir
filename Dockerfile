...
# pandoc (markdown→PDF/HTML conversion for report attachments), ffmpeg (v4l2
# frame capture for the webcam skill)
RUN apt-get update && \
    apt-get install -y --no-install-recommends ripgrep git jq curl pandoc texlive-latex-recommended lmodern nodejs npm ffmpeg && \
