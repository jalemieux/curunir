...
- Convert with the shared helper, which is already installed in the environment: `python -m src.md2pdf file.md` (writes `file.pdf`). It sanitizes LaTeX-hostile emoji like ✅/❌ — the glyphs your own output routinely uses — then renders via pandoc with xelatex (falling back to pdflatex), so the render no longer crashes on them. Reach for it on the first attempt instead of raw `pandoc`. Do not render via HTML, headless Chromium, or CSS. If PDF conversion fails, attach the `.md` as a fallback.
- Never `pip install` (or otherwise install packages at runtime) to produce a deliverable. The tooling you need — `src.md2pdf`, pandoc, and LaTeX — is already in the image. The environment is ephemeral, so a runtime install is slow, non-deterministic, and gone next session.
